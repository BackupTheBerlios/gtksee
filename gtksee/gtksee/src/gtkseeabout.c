...
get_about_dialog()
{
   ...
   big_font = gdk_font_load(("-*-helvetica-bold-r-normal--*-160-*-*-*-*-*-*"));
   ...
   snprintf(buf, 15, "GTK See v%s", VERSION);
   ...
   gtk_widget_set_style(widget, style);
	...
	widget = gtk_label_new(_("\n(c)1998-? Lee Luyang(Hotaru), China"));
	gtk_widget_show(widget);
	gtk_box_pack_start(GTK_BOX(box), widget, FALSE, FALSE, 0);
	...
	widget = gtk_label_new(_("See the 'COPYING' file for details\n"));
	gtk_widget_show(widget);
	gtk_box_pack_start(GTK_BOX(box), widget, FALSE, FALSE, 0);
